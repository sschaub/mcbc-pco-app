...
with open("../notion_export.csv", newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for tag_group in TAG_GROUPS:
            if tag_group not in reader.fieldnames:
                print(f"ERROR: tag group '{tag_group}' is not one of the CSV fields.")
                exit(1)
        for row in reader:
            # pprint(row)
            title = row["title"]
            ...
            # print(f"Processing Song #{song_number}, Arr #{arr_number}")
            ...
            try:
                ...
                for tag_group in TAG_GROUPS:
                    ...
                    for tag_name in tag_names:
                        tag_id = all_tags.get(tag_group.lower(), {}).get(tag_name.lower())
                        if tag_id:
                            arr_tag_ids.append(tag_id)
                        else:
                            print(f'Song {song_number} Arr {arr_number}: tag "{tag_name}" not defined in group "{tag_group}"')
                ...
                if ex.status_code == 404:
                     print(f"SKIPPING Song {song_number} Arr {arr_number}: No PCO arrangement found for song '{title}'")
                     continue
                else:
                     raise
